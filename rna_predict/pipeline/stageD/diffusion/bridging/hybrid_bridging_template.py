...
def hybrid_bridging_sparse_to_dense(
    ...
    Given a sparse atom feature tensor and a residue-to-canonical atom mapping, this function constructs a dense tensor of atom features and a boolean mask indicating valid atom positions, supporting both flat and per-residue sparse input formats.
    
    Args:
        stage_c_output (torch.Tensor): Sparse atom features from Stage C, shape either [batch_size, n_residues, n_sparse_atoms, feature_dim] or [batch_size, total_sparse_atoms, feature_dim].
        residue_atom_map (List[List[int]]): Maps each residue's sparse atom indices to canonical atom indices.
        n_residues (int): Number of residues.
        canonical_atom_count (int): Number of canonical atom positions per residue.
        ...
        debug_logging (bool, optional): Enables debug logging if True.
    
    Returns:
        Tuple[torch.Tensor, torch.Tensor]: 
            - Dense atom tensor of shape [batch_size, n_residues, canonical_atom_count, feature_dim].
            - Boolean mask of shape [batch_size, n_residues, canonical_atom_count] indicating valid atoms.
    
    Raises:
        ValueError: If the input tensor shape does not match the mapping or is unsupported.
    """
