...
TEST CODE: 
# ----- test_run_stageD_diffusion_basic.py -----
import run_stageD
import torch
import unittest
...
class TestFuzzRun_Staged_Diffusion(unittest.TestCase):

    @given(partial_coords=st.builds(Tensor), trunk_embeddings=st.builds(dict), diffusion_config=st.builds(dict), mode=st.text(), device=st.text())
    def test_fuzz_run_stageD_diffusion(self, partial_coords: torch.Tensor, trunk_embeddings: dict, diffusion_config: dict, mode: str, device: str) -> None:
        run_stageD.run_stageD_diffusion(partial_coords=partial_coords, trunk_embeddings=trunk_embeddings, diffusion_config=diffusion_config, mode=mode, device=device)
