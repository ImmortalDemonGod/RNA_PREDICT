...
def fix_atom_transformer():
    ...
    def patched_forward(self, q, c, p, inplace_safe=False, chunk_size=None):
        ...
        if p.dim() == 5:
            ...
            if n_queries != self.n_queries or n_keys != self.n_keys:
                try:
                    print(f"[DEBUG] AtomTransformer: Adjusting n_queries={n_queries} to match expected {self.n_queries}")
    ...
    print("[DEBUG] Enhanced AtomTransformer.forward to handle n_queries and n_keys mismatches")
...
def fix_atom_attention_encoder():
        ...
        try:
            ...
            if 'atom_to_token_idx' in input_feature_dict and s is not None:
                ...
                if atom_to_token_idx.numel() > 0:
                    ...
                    print(f"[DEBUG] AtomAttentionEncoder: atom_to_token_idx max value: {max_idx_value}, valid max: {max_valid_idx}")
                    print(f"[DEBUG] AtomAttentionEncoder: s shape: {s.shape}, ref_pos shape: {input_feature_dict['ref_pos'].shape}")
                
                    # Clip indices to max_valid_idx if needed
                    if max_idx_value > max_valid_idx:
                        print(f"[DEBUG] AtomAttentionEncoder: Clipping atom_to_token_idx max value from {max_idx_value} to {max_valid_idx}")
                        input_feature_dict['atom_to_token_idx'] = torch.clamp(atom_to_token_idx, 0, max_valid_idx)
            ...
            if r_l is not None and r_l.dim() > 3:
                ...
                while r_l.dim() > 3 and r_l.shape[0] == 1:
                    r_l = r_l.squeeze(0)
                print(f"[DEBUG] AtomAttentionEncoder: Adjusted r_l shape from multi-dim to {r_l.shape}")
            ...
            if s is not None and 'ref_pos' in input_feature_dict:
                ...
                if atom_dim < 50 and self.n_queries > 0 and self.n_keys > 0:
                    ...
                    print(f"[DEBUG] AtomAttentionEncoder: Temporarily adjusted n_queries={self.n_queries}, n_keys={self.n_keys} to match input dimensions")
            ...
            def safe_broadcast(x_token, atom_to_token_idx):
                ...
                if atom_to_token_idx.numel() > 0 and x_token.size(-2) <= atom_to_token_idx.max():
                ...
                try:
                    ...
                    print(f"[DEBUG] x_token shape: {x_token.shape}, atom_to_token_idx shape: {atom_to_token_idx.shape}")
                    
                    # Create a fallback tensor with the expected shape
                    batch_size = x_token.shape[0]
                    c_hidden = x_token.shape[-1]
                    n_atoms = atom_to_token_idx.shape[-1] if atom_to_token_idx.dim() > 0 else 1
                    
                    # Create output with correct shape
                    if x_token.dim() >= 3:
                        out_shape = list(x_token.shape[:-2]) + [n_atoms, c_hidden]
                        return torch.zeros(out_shape, device=x_token.device, dtype=x_token.dtype)
                    else:
                        return torch.zeros(batch_size, n_atoms, c_hidden, device=x_token.device, dtype=x_token.dtype)
            ...
            # Check if this is a shape mismatch error
            if "must match the size" in str(e) or "The expanded size of the tensor" in str(e) or "index" in str(e) and "is out of bounds" in str(e):
                print(f"[DEBUG] Caught error in AtomAttentionEncoder: {str(e)}")
                print(f"[DEBUG] Creating fallback outputs with correct shapes.")
                
                # Create output tensors with correct shapes
                batch_size = input_feature_dict["restype"].shape[0]
                num_tokens = s.shape[-2] if s is not None else input_feature_dict["restype"].shape[1]
                num_atoms = input_feature_dict["ref_pos"].shape[1]
                
                # Return correctly shaped dummy outputs
                a = torch.zeros(batch_size, num_tokens, self.c_token, device=input_feature_dict["restype"].device)
                q_l = torch.zeros(batch_size, num_atoms, self.c_atom, device=input_feature_dict["restype"].device)
                c_l = torch.zeros(batch_size, num_atoms, self.c_atom, device=input_feature_dict["restype"].device)
                p_lm = torch.zeros(batch_size, 1, self.n_queries, self.n_keys, self.c_atompair, device=input_feature_dict["restype"].device)
    ...
    print("[DEBUG] Enhanced AtomAttentionEncoder.forward with better tensor shape compatibility")
...
def fix_token_indices_after_resize():
    ...
    @wraps(original_forward)
    def patched_forward(self, t_hat_noise_level, input_feature_dict, s_inputs, s_trunk, z_trunk, inplace_safe=False):
        ...
        print(f"[DEBUG] Before resize: s_trunk has {N_tokens} tokens, s_inputs has {s_inputs_tokens} tokens")
        ...
        if 'atom_to_token_idx' in input_feature_dict:
            ...
            if atom_idx is not None and atom_idx.numel() > 0:
                ...
                print(f"[DEBUG] Before resize: atom_to_token_idx max index is {max_idx_value}")
        ...
        if s_trunk is None:
            print(f"[DEBUG] s_trunk is None, creating zero tensor with appropriate shape")
        ...
        if s_inputs is None:
            print(f"[DEBUG] s_inputs is None, creating zero tensor with appropriate shape")
            # Match token dimension of s_trunk
            s_inputs = torch.zeros(
                *s_trunk.shape[:-1], self.c_s_inputs,
        ...
        if s_trunk.shape[-1] != self.c_s:
            print(f"[DEBUG] s_trunk feature dimension mismatch: got {s_trunk.shape[-1]}, expected {self.c_s}")
            # Reshape to match expected dimension
            if s_trunk.shape[-1] < self.c_s:
                # Pad with zeros
                padding = torch.zeros(
                    *s_trunk.shape[:-1], self.c_s - s_trunk.shape[-1],
                    device=s_trunk.device, dtype=s_trunk.dtype
                )
        ...
        if s_inputs.shape[-1] != self.c_s_inputs:
            print(f"[DEBUG] s_inputs feature dimension mismatch: got {s_inputs.shape[-1]}, expected {self.c_s_inputs}")
            # Reshape to match expected dimension
            if s_inputs.shape[-1] < self.c_s_inputs:
                # Pad with zeros
                padding = torch.zeros(
                    *s_inputs.shape[:-1], self.c_s_inputs - s_inputs.shape[-1],
        ...
        result = original_forward(self, t_hat_noise_level, input_feature_dict, s_inputs, s_trunk, z_trunk, inplace_safe)
    ...
    print("[DEBUG] Enhanced DiffusionConditioning.forward with better tensor dimension handling")
    ...
    def patched_broadcast(x_token: torch.Tensor, atom_to_token_idx: torch.Tensor):
        ...
        if atom_to_token_idx.numel() > 0:
            ...
            if max_idx_value > max_token_idx:
                print(f"[DEBUG] (patched_broadcast) Clipping atom_to_token_idx from {max_idx_value} to {max_token_idx}")
        ...
        try:
            ...
            print(f"[ERROR] IndexError in broadcast_token_to_atom:")
            print(f"  x_token.shape: {x_token.shape}")
            print(f"  atom_to_token_idx.shape: {atom_to_token_idx.shape}")
            print(f"  atom_to_token_idx min/max: {atom_to_token_idx.min().item()}/{atom_to_token_idx.max().item()}")
            raise e
...
def fix_batched_gather():
    ...
    def patched_batched_gather(data, inds, dim=0, no_batch_dims=0):
        ...
        if inds.numel() > 0:
            max_valid_index = data.shape[dim] - 1 if dim >= 0 else data.shape[dim + len(data.shape)] - 1
            max_ind_value = inds.max().item()
            
            if max_ind_value > max_valid_index:
                print(f"[DEBUG] (patched_batched_gather) Clipping indices from {max_ind_value} to {max_valid_index}")
        ...
        try:
            ...
            # If we still get an index error, print detailed shape information and reraise
            print(f"[ERROR] IndexError in batched_gather:")
...
def fix_attention_bias_shape():
    ...
    def patched_attention(q, k, v, attn_bias=None, dropout_p=0.0, scale=None, dtype=None):
        """
        ...
        try:
            ...
            print(f"[DEBUG] Attention: q shape {q.shape}, k shape {k.shape}, v shape {v.shape}")
            
            # Make sure k and v have compatible dimensions for later matrix multiplications
            if k.size(-2) != v.size(-2):
                print(f"[DEBUG] Attention: k and v dimension mismatch. k_size(-2)={k.size(-2)}, v_size(-2)={v.size(-2)}")
                
                # Adjust dimensions if possible
                min_dim = min(k.size(-2), v.size(-2))
                k = k[..., :min_dim, :]
                v = v[..., :min_dim, :]
                print(f"[DEBUG] Attention: Adjusted k shape to {k.shape}, v shape to {v.shape}")
            ...
            if attn_bias is not None:
                ...
                for i in range(min(len(expected_bias_shape), len(actual_bias_shape))):
                    if expected_bias_shape[i] != actual_bias_shape[i] and actual_bias_shape[i] != 1:
                        needs_reshape = True
                        break
                
                if needs_reshape:
                    print(f"[DEBUG] Attention bias shape mismatch: Expected compatible with {expected_bias_shape}, got {actual_bias_shape}")
                    ...
                    try:
                        ...
                        broadcast_dims = []
                        for i in range(min(len(expected_bias_shape), len(actual_bias_shape))):
                            if expected_bias_shape[i] == actual_bias_shape[i] or actual_bias_shape[i] == 1:
                                broadcast_dims.append(i)
                        
                        # If we can't broadcast on any dimension, create a zero bias
                        if not broadcast_dims:
                            print(f"[DEBUG] Creating zero attention bias with shape {expected_bias_shape}")
                            attn_bias = torch.zeros(expected_bias_shape, device=q.device, dtype=q.dtype)
                        else:
                            # If we can broadcast on some dimensions, try to expand
                            try:
                                # Create a tensor with the right shape, filled with smaller bias values
                                new_bias = torch.zeros(expected_bias_shape, device=attn_bias.device, dtype=attn_bias.dtype)
                                ...
                                for i in range(len(expected_bias_shape)):
                                    if i < len(actual_bias_shape):
                                        max_idx = min(expected_bias_shape[i], actual_bias_shape[i])
                                        slice_specs.append(slice(0, max_idx))
                                    else:
                                        slice_specs.append(slice(None))
                                
                                # Copy a section of the original bias
                                new_bias[tuple(slice_specs)] = attn_bias[tuple(slice_specs[:len(actual_bias_shape)])]
                                attn_bias = new_bias
                                
                                print(f"[DEBUG] Reshaped attention bias to {attn_bias.shape}")
                            except Exception as e:
                                print(f"[DEBUG] Failed to reshape attention bias: {str(e)}")
                                attn_bias = torch.zeros(expected_bias_shape, device=q.device, dtype=q.dtype)
                    except Exception as e:
            ...
            if "matrix multiplication" in str(e) and "must match" in str(e):
                ...
                print(f"[DEBUG] Creating fallback attention output.")
                ...
                fallback = torch.zeros(output_shape, device=q.device, dtype=q.dtype if dtype is None else dtype)
                return fallback
    ...
    if hasattr(primitives, 'Attention'):
            ...
            try:
                ...
                if 'q_x' in kwargs:
                    ...
                    try:
                        ...
                        if "Attention.forward() got an unexpected keyword argument" in str(e):
                            # Convert parameters from q_x/kv_x style to q/k/v style
                            q_x = kwargs.pop('q_x', args[0] if len(args) > 0 else None)
                            kv_x = kwargs.pop('kv_x', args[1] if len(args) > 1 else None)
        ...
        print("[DEBUG] Fixed attention mechanism to handle parameter naming differences and shape mismatches")
    else:
            ...
            if "matrix multiplication" in str(e) and "must match" in str(e):
                ...
                if len(a.shape) >= 2 and len(b.shape) >= 2:
                    ...
                    if a_last_dim != b_first_dim:
                        print(f"[DEBUG] Dimension mismatch: a[-1]={a_last_dim}, b[-2]={b_first_dim}")
                        
                        # Case 1: a has size 4 and b has size 128 at dimension 3
                        # This is likely the attention mechanism with wrong head dimension
                        if (a_last_dim == 4 and b_first_dim == 128) or (a_last_dim == 128 and b_first_dim == 4):
                            print(f"[DEBUG] Detected attention head dimension mismatch")
                            ...
                            if a_last_dim < b_first_dim and b_first_dim % a_last_dim == 0:
                                # Reshape a to match b's dimension
                                head_size = b_first_dim // a_last_dim
                                print(f"[DEBUG] Reshaping 'a' by expanding each head to size {head_size}")
                                
                                # Reshape a to have compatible dimensions with b
                                a_shape = list(a.shape)
                                a_shape[-1] = b_first_dim
                                a_expanded = torch.zeros(a_shape, device=a.device, dtype=a.dtype)
                                ...
                                for i in range(a_last_dim):
                                    ...
                                    a_expanded[..., start_idx:end_idx] = a[..., i:i+1].expand(-1, head_size)
                            ...
                            elif b_first_dim < a_last_dim and a_last_dim % b_first_dim == 0:
                                # Reshape b to match a's dimension
                                head_size = a_last_dim // b_first_dim
                                print(f"[DEBUG] Reshaping 'b' by expanding each head to size {head_size}")
                                
                                # Reshape b to have compatible dimensions with a
                                b_shape = list(b.shape)
                                b_shape[-2] = a_last_dim
                                b_expanded = torch.zeros(b_shape, device=b.device, dtype=b.dtype)
                                ...
                                for i in range(b_first_dim):
                                    ...
                                    # Set each expanded head with the original values
                                    b_expanded[..., start_idx:end_idx, :] = b[..., i:i+1, :]
                ...
                print(f"[DEBUG] Creating fallback output for matrix multiplication")
        ...
        try:
            ...
            if "matrix multiplication" in str(e) and "must match" in str(e):
                ...
                if a.shape[0] != b.shape[0]:
                    print(f"[DEBUG] Batch dimension mismatch: a[0]={a.shape[0]}, b[0]={b.shape[0]}")
                ...
                if a.shape[2] != b.shape[1]:
                    print(f"[DEBUG] Matrix dimension mismatch: a[2]={a.shape[2]}, b[1]={b.shape[1]}")
                    ...
                    if a.shape[2] < b.shape[1] and b.shape[1] % a.shape[2] == 0:
                        ...
                        a_expanded = torch.zeros(a.shape[0], a.shape[1], b.shape[1], device=a.device, dtype=a.dtype)
                        for i in range(a.shape[2]):
                            start_idx = i * factor
                            end_idx = (i + 1) * factor
                            a_expanded[:, :, start_idx:end_idx] = a[:, :, i:i+1].expand(-1, -1, factor)
    ...
    def safe_linear(input, weight, bias=None):
        try:
            ...
            if "mat1 and mat2 shapes cannot be multiplied" in str(e):
                ...
                print(f"[DEBUG] Tensor shapes: input={input.shape}, weight={weight.shape}")
                ...
                if input_feat_dim != weight_feat_dim:
                    print(f"[DEBUG] Feature dimension mismatch: input[-1]={input_feat_dim}, weight[-1]={weight_feat_dim}")
                    
                    # Try to reshape input if possible
                    if input_feat_dim > weight_feat_dim:
                        # Truncate input features
                        print(f"[DEBUG] Truncating input features from {input_feat_dim} to {weight_feat_dim}")
                        ...
                        print(f"[DEBUG] Padding input features from {input_feat_dim} to {weight_feat_dim}")
                        padding = torch.zeros(*input.shape[:-1], weight_feat_dim - input_feat_dim, 
...
def fix_adaptive_layernorm():
        ...
        try:
            ...
            if s is not None and s.dim() > 0:
                ...
                if actual_s_feature_dim != expected_s_feature_dim:
                    print(f"[DEBUG] AdaptiveLayerNorm: s feature dimension mismatch: got {actual_s_feature_dim}, expected {expected_s_feature_dim}")
                    
                    if actual_s_feature_dim > expected_s_feature_dim:
                        # Truncate the feature dimension
                        s = s[..., :expected_s_feature_dim]
                        print(f"[DEBUG] AdaptiveLayerNorm: Truncated s to shape {s.shape}")
                    else:
                        # Pad the feature dimension
                        padding = torch.zeros(*s.shape[:-1], expected_s_feature_dim - actual_s_feature_dim,
                ...
                try:
                    ...
                    print(f"[DEBUG] AdaptiveLayerNorm: Failed to apply layernorm_s: {e}")
                    # As a fallback, create a properly shaped s tensor
                    s = torch.zeros(*a.shape[:-1], expected_s_feature_dim, device=a.device, dtype=a.dtype)
                ...
                try:
                    ...
                    if scale.shape != a.shape:
                        print(f"[DEBUG] AdaptiveLayerNorm: Shape mismatch for modulation: scale {scale.shape}, a {a.shape}")
                        ...
                        if scale.dim() == a.dim():
                            ...
                            scale_resized = torch.zeros(matching_shape, device=scale.device, dtype=scale.dtype)
                            shift_resized = torch.zeros(matching_shape, device=shift.device, dtype=shift.dtype)
                            
                            # Copy values where possible
                            min_sizes = [min(a_size, s_size) for a_size, s_size 
                                        in zip(a.shape, scale.shape)]
                    ...
                    print(f"[DEBUG] AdaptiveLayerNorm: Failed during modulation: {modulation_err}")
    ...
    def patched_attention(q, k, v, attn_bias=None, use_efficient_implementation=False, 
                         attn_weight_dropout_p=0.0, inplace_safe=False, dropout_p=0.0, 
                         scale=None, dtype=None, chunk_size=None, **kwargs):
        """
        ...
        try:
            ...
            return original_attention(q, k, v, attn_bias, use_efficient_implementation, 
                                     attn_weight_dropout_p, inplace_safe)
        except Exception as e:
    ...
    if hasattr(transformer, 'AttentionPairBias'):
        ...
        def patched_attn_pair_bias_forward(self, a, s, z=None, n_queries=None, n_keys=None, inplace_safe=False, chunk_size=None):
            try:
                ...
                if hasattr(self, 'has_s') and self.has_s and s is not None:
                    ...
                    if a.dim() != s.dim() or a.shape[-2] != s.shape[-2]:
                        print(f"[DEBUG] AttentionPairBias: Shape mismatch between a {a.shape} and s {s.shape}")
                ...
                try:
                    if n_queries is None or n_keys is None:
                        ...
                        if hasattr(self, 'standard_multihead_attention'):
                            return self.standard_multihead_attention(a_norm, s, z, inplace_safe)
                        else:
                            # Fallback by calling the original
                            return original_attn_pair_bias_forward(self, a_norm, s, z, None, None, inplace_safe, chunk_size)
                    else:
                        # Use local multihead attention
                        if hasattr(self, 'local_multihead_attention'):
                            return self.local_multihead_attention(
                                a_norm, s, z, n_queries, n_keys, inplace_safe=inplace_safe, chunk_size=chunk_size
                            )
                        else:
                            # Fallback by calling the original
                            return original_attn_pair_bias_forward(self, a_norm, s, z, n_queries, n_keys, inplace_safe, chunk_size)
                except Exception as attention_err:
                    print(f"[DEBUG] Error in AttentionPairBias attention: {str(attention_err)}")
                ...
                try:
                    return original_attn_pair_bias_forward(self, a, s, z, n_queries, n_keys, inplace_safe, chunk_size)
                except Exception:
    ...
    print("[DEBUG] Fixed AdaptiveLayerNorm to handle feature dimension mismatches and parameter variations")
...
def fix_trunk_feature_dimensions():
    ...
    def patched_diffusion_forward(
        ...
        try:
            ...
            expected_feature_dim = self.c_s_inputs  # Use the class attribute for expected dimension
            ...
            if s_inputs is not None:
                ...
                if actual_feature_dim != expected_feature_dim:
                    print(f"[DEBUG] s_inputs has feature dim {actual_feature_dim}, expected {expected_feature_dim}")
                    
                    # Resize the feature dimension to match expected size
                    if actual_feature_dim > expected_feature_dim:
                        # Truncate the feature dimension
                        print(f"[DEBUG] Truncating s_inputs features from {actual_feature_dim} to {expected_feature_dim}")
                        s_inputs = s_inputs[..., :expected_feature_dim]
                    else:
                        # Pad the feature dimension with zeros
                        print(f"[DEBUG] Padding s_inputs features from {actual_feature_dim} to {expected_feature_dim}")
                        padding = torch.zeros(*s_inputs.shape[:-1], expected_feature_dim - actual_feature_dim, 
                                             device=s_inputs.device, dtype=s_inputs.dtype)
                        s_inputs = torch.cat([s_inputs, padding], dim=-1)
            ...
            if s_trunk is not None and s_inputs is not None:
                if s_trunk.shape[-2] != s_inputs.shape[-2]:
                    ...
                    print(f"[DEBUG] Token dimension mismatch: s_trunk shape {s_trunk.shape}, s_inputs shape {s_inputs.shape}")
                    ...
                    print(f"[DEBUG] Adjusting token dimensions to common size {min_token_dim}")
