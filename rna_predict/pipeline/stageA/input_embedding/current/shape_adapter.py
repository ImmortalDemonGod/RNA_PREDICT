...
def adapt_tensors_for_addition(tensor_a: torch.Tensor, tensor_b: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for i in range(max_dims):
        if padded_shape_a[i] != padded_shape_b[i] and padded_shape_a[i] != 1 and padded_shape_b[i] != 1:
    ...
    if mismatch_dims:
        ...
        if len(shape_a) >= 6 and len(shape_b) >= 6:
            ...
            tensor_b = tensor_b.mean(dim=3, keepdim=True).expand(-1, -1, -1, shape_a[3], -1, -1)
            tensor_b = tensor_b.mean(dim=4, keepdim=True).expand(-1, -1, -1, -1, shape_a[4], -1)
            tensor_b = tensor_b.mean(dim=5, keepdim=True).expand(-1, -1, -1, -1, -1, shape_a[5], -1)
