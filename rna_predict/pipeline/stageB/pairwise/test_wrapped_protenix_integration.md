...
TEST CODE: 
# ----- test_ProtenixIntegration_basic.py -----
import protenix_integration
import unittest
from hypothesis import given, strategies as st

class TestFuzzProtenixintegration(unittest.TestCase):

    @given(c_token=st.just(384), restype_dim=st.just(32), profile_dim=st.just(32), c_atom=st.just(128), c_pair=st.just(32), num_heads=st.just(4), num_layers=st.just(3), use_optimized=st.booleans(), device=st.just(device(type='cpu')))
    def test_fuzz_ProtenixIntegration(self, c_token, restype_dim, profile_dim, c_atom, c_pair, num_heads, num_layers, use_optimized, device) -> None:
        protenix_integration.ProtenixIntegration(c_token=c_token, restype_dim=restype_dim, profile_dim=profile_dim, c_atom=c_atom, c_pair=c_pair, num_heads=num_heads, num_layers=num_layers, use_optimized=use_optimized, device=device)

# ----- test_ProtenixIntegration_build_embeddings_errors.py -----
import protenix_integration
import unittest
from hypothesis import given, reject, strategies as st

class TestFuzzProtenixintegrationbuild_Embeddings(unittest.TestCase):

    @given(self=st.nothing(), input_features=st.builds(dict))
    def test_fuzz_ProtenixIntegration_build_embeddings(self, input_features: dict) -> None:
        try:
            protenix_integration.ProtenixIntegration.build_embeddings(self=self, input_features=input_features)
        except (TypeError, ValueError):
            reject()

# ----- test_ProtenixIntegration_build_embeddings_basic.py -----
import protenix_integration
import unittest
from hypothesis import given, strategies as st
