...
class StageBTorsionBertPredictor(nn.Module):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        Configures the predictor using a Hydra configuration object, handling device assignment, model and tokenizer loading, dummy mode fallback for testing or incomplete configuration, and optional LoRA/PEFT integration. Supports multiple configuration structures and enforces explicit device specification. Sets model parameters such as angle mode, number of angles, and output dimension based on the configuration. Moves the model to the specified device, applies LoRA if enabled, and sets the model to evaluation mode. In dummy mode, instantiates a placeholder model for test environments or missing configuration.
        """
        super().__init__()
    ...
    def _preprocess_sequence(self, sequence: str) -> Dict[str, torch.Tensor]:
        ...
        If the tokenizer is unavailable or tokenization fails, returns dummy input tensors with appropriate shapes on the correct device.
        """
    ...
    def predict_angles_from_sequence(self, sequence: str, stochastic_pass: bool = False, seed: Optional[int] = None) -> torch.Tensor:
        ...
        Tokenizes the input RNA sequence, prepares model inputs, and performs inference to obtain predicted torsion angles for each residue. Handles device placement, model mode switching for stochastic inference (e.g., MC dropout) if requested, and output postprocessing to ensure the result matches the expected shape and dimension. Returns a dummy tensor if the model is mocked or the sequence is empty.
        
        Args:
            sequence: RNA sequence as a string.
            stochastic_pass: If True, enables stochastic inference by setting the model to training mode (e.g., for MC dropout).
            seed: Optional random seed for reproducibility during stochastic inference.
        
        Returns:
            A tensor of shape [num_residues, output_dim] containing predicted torsion angles for each residue in the sequence.
        """
        if self.debug_logging:
    ...
    def _convert_sincos_to_angles(
        ...
        """
        Converts a tensor of sine and cosine pairs to angles in radians or degrees.
    ...
    def __call__(self, sequence: str, adjacency=None, stochastic_pass: bool = False, seed: Optional[int] = None, **kwargs: Any) -> Dict[str, torch.Tensor]:
        ...
        Supports stochastic inference when `stochastic_pass` is enabled, optionally using a provided random seed. In dummy mode, returns zero-filled tensors with appropriate shapes for testing or missing configuration scenarios.
        ...
        Args:
            ...
            stochastic_pass: If True, enables stochastic inference (e.g., MC dropout).
