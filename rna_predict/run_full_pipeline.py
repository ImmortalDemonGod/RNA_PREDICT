...
try:
    ...
    print("[Warning] Stage D modules could not be imported. Stage D functionality will be disabled.")
...
class SimpleLatentMerger(torch.nn.Module):
    ...
    def forward(self,
        ...
        if self.mlp[0].in_features != total_in_dim:
            print(f"[Debug] Creating MLP with dimensions: {total_in_dim} -> {self.dim_out}")
            self.mlp = torch.nn.Sequential(
...
def run_full_pipeline(sequence: str, config: Dict[str, Any], device: str = "cuda") -> Dict[str, Any]:
    """
    ...
    if not torsion_model or not pairformer_model:
        raise ValueError("Need both 'torsion_bert_model' and 'pairformer_model' in config.")
    ...
    if config.get("enable_stageC", False):
        ...
        partial_coords = partial_res["coords"]  # e.g., shape [N * #atoms, 3] or [N, #atoms, 3]

    # 4) Optional unify/merge
    unified_latent = None
    if config.get("merge_latent", False):
        if "merger" not in config:
            raise ValueError("Config sets 'merge_latent' but no 'merger' object provided.")
        merger_module = config["merger"]
        
        # Print tensor shapes for debugging
        print(f"[Debug] Tensor shapes for merger:")
    ...
    if config.get("run_stageD", False) and STAGE_D_AVAILABLE:
        ...
        if not diffusion_manager:
            raise ValueError("Config indicates 'run_stageD' but no 'diffusion_manager' provided.")
...
if __name__ == "__main__":
    ...
    try:
        tmodel = StageBTorsionBertPredictor(model_name_or_path="sayby/rna_torsionbert", device=dev)
    except Exception as e:
        print(f"[Warning] Could not load 'sayby/rna_torsionbert'. Using dummy path. Error: {str(e)}")
        tmodel = StageBTorsionBertPredictor(model_name_or_path="dummy_path", device=dev)

    pfmodel = PairformerWrapper(n_blocks=2, c_z=32, c_s=64, use_checkpoint=False).to(dev)
