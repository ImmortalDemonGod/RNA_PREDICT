...
import logging
import sys # To potentially exit gracefully
import cv2 # For potential debugging saves

# Import local modules using relative paths
from .logger import setup_logger
from .config import TEMPLATE_PATH, THRESHOLD, LOGGING_LEVEL
...
def find_template_on_screen(logger, template_path, threshold):
    ...
    for i, monitor_data in enumerate(monitor_screenshots):
        ...
        logger.info(f"Performing template matching on {monitor_dims_str} using template: '{template_path}' with threshold: {threshold}")
        ...
        if match_result:
            match_location_relative, correlation_score = match_result # Unpack the result
            ...
            logger.info(f"Template found on {monitor_dims_str} at relative coordinates {match_location_relative}, absolute coordinates {absolute_match_location} with correlation {correlation_score:.4f}")
            # Return the full monitor_info dictionary along with coordinates and correlation
            return {'found': True, 'coordinates': absolute_match_location, 'monitor_info': monitor_info, 'correlation': correlation_score}
...
def main():
    ...
    logger.debug(f"Configuration: TEMPLATE_PATH='{TEMPLATE_PATH}', THRESHOLD={THRESHOLD}, LOGGING_LEVEL='{log_level_str}'")
    ...
    if result['found']:
        ...
        logger.info(f"Template successfully found on {monitor_dims_str} at absolute screen coordinates: {coords}")
        print(f"Success: Template found on {monitor_dims_str} at absolute screen coordinates: {coords}")
    else:
