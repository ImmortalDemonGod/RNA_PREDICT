...
def calculate_rna_torsions_mdanalysis(pdb_file, chain_id="A", fallback=False):
    ...
    if len(rna_chain) == 0:
        if fallback:
            print(
                f"No atoms found with segid/chainID={chain_id}, falling back to all nucleic."
            )
            rna_chain = u.select_atoms("nucleic")
        else:
            raise ValueError(
                f"No atoms found for chain/segment='{chain_id}'. Check PDB labeling."
            )
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python mdanalysis_torsion_example.py <rna_structure.pdb> [chainID]"
        )
        sys.exit(1)
