    ...
    for part in parts:
        ...
        if "-" in part:
            try:
                ...
                if len(range_parts) == 2:
                    ...
                    if start > end:  # Handle cases like '10-5' if they occur
                        print(
                            f"Warning: Correcting inverted range '{part}' to '{end}-{start}'.",
                            file=sys.stderr,
                        )
                        start, end = end, start
                    ...
                    print(
                        f"Warning: Could not parse range '{part}' in missing lines string.",
                        file=sys.stderr,
                    )
            ...
            try:
                ...
                print(
                    f"Warning: Could not parse line number '{part}' in missing lines string.",
                    file=sys.stderr,
                )
    return lines
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run tests with coverage and memory profiling"
    )
    parser.add_argument(
        "--least-covered", action="store_true", help="Show least covered files"
    )
    parser.add_argument(
        "--filter", type=str, help="Filter coverage report by file pattern"
    )
...
def show_least_covered():
    ...
    if most_uncovered_file:
        coverage_percentage = (
            100 - (max_uncovered_lines * 100 // total_statements)
                ...
                if most_uncovered_lines:
                    ...
                    for line_num in sorted(missing_line_nums):
                        if 1 <= line_num <= len(source_lines):
                            print(
                                f"{line_num:4d} | {source_lines[line_num - 1].rstrip()}"
                            )
