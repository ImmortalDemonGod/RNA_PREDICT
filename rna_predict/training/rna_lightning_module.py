...
class RNALightningModule(L.LightningModule):
    ...
    def _instantiate_pipeline(self, cfg):
        ...
        Validates the configuration and device specification, handles checkpoint downloading and extraction for Stage A, and instantiates each pipeline stage on the specified device. Aggregates all stages into a ModuleDict for unified parameter management and ensures all components are ready for training or inference.
        """
    ...
    def forward(self, batch, **kwargs):
        """
        Runs a forward pass through the RNA_PREDICT pipeline for a given input batch.
        
        In integration test mode, returns dummy outputs with the expected structure. Otherwise, processes the input through all pipeline stages: predicts torsion angles and embeddings, reconstructs 3D coordinates, merges latent representations, and returns a dictionary containing intermediate and final outputs. All tensors are placed on the configured device, and atom-level metadata is included if available.
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        Performs a single training step for the RNA_PREDICT pipeline, computing losses and managing conditional execution of pipeline stages.
        
        This method computes the torsion angle loss and, depending on configuration, may also compute coordinate loss from Stage C outputs or execute Stage D for diffusion-based refinement. It handles device placement, input validation, bridging between residue and atom-level features, and supports streamlined and test modes. If Stage D is disabled, returns a zero loss with a valid gradient path.
        
        Returns:
            A dictionary containing the computed loss and, if applicable, Stage D results.
        """
        if not self.cfg.run_stageD:
        ...
        if hasattr(self, 'debug_logging') and self.debug_logging:
            ...
            def log_param_devices(module, name):
                ...
                Args:
                    module: The PyTorch module whose parameters will be inspected.
                    name: A label used in the log output to identify the module context.
                """
                for pname, param in module.named_parameters(recurse=True):
