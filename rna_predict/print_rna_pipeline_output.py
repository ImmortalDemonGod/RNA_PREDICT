...
from rna_predict.run_full_pipeline import run_full_pipeline, SimpleLatentMerger
...
def setup_pipeline():
    ...
    # Create models
    try:
        tmodel = StageBTorsionBertPredictor(model_name_or_path="sayby/rna_torsionbert", device=device)
    except Exception as e:
        print(f"[Warning] Could not load 'sayby/rna_torsionbert'. Using dummy model.")
    ...
    pfmodel = PairformerWrapper(n_blocks=2, c_z=c_z, c_s=c_s, use_checkpoint=False).to(device)
    
    # Create merger
    merger = SimpleLatentMerger(dim_angles=14, dim_s=c_s, dim_z=c_z, dim_out=128).to(device)
    ...
    if STAGE_D_AVAILABLE:
        ...
        diffusion_manager = ProtenixDiffusionManager(dummy_diffusion_config, device=device)
