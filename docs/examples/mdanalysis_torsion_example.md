...
def calculate_rna_torsions_mdanalysis(
    pdb_file, chain_id="A", fallback=False, csv_file=None
):
    """
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python mdanalysis_torsion_example.py <rna_structure.[pdb|cif]> [chainID]"
        )
        sys.exit(1)
