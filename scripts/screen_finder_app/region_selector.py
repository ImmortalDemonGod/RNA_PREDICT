...
def on_button_press(event):
    ...
    if rect:
        ...
        rect = canvas.create_rectangle(
            start_x, start_y, start_x, start_y, outline="red", width=2, dash=(4, 2)
        )
    logger.info(
        f"Selection started at screen coordinates ({event.x_root}, {event.y_root}), canvas coordinates ({start_x}, {start_y})"
    )
...
def on_button_release(event):
    ...
    logger.info(
        f"Selection completed. Canvas coords: TopLeft=({final_x}, {final_y}), W={width}, H={height}"
    )
...
def select_region() -> Optional[list]:
    ...
    try:
        ...
        logger.error(
            f"Could not get screen dimensions using PIL: {e}. Falling back to Tkinter info."
        )
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        logger.warning(
            f"Using Tkinter screen dimensions: {screen_width}x{screen_height}"
        )
    ...
    label = tk.Label(
        root,
        text="Click and drag to select a region. Release to confirm.",
        bg="yellow",
        fg="black",
        font=("Arial", 12),
    )
...
if __name__ == "__main__":
    ...
    if selected_area:
        print(
            f"Selected Region Coordinates: x={selected_area[0]}, y={selected_area[1]}, width={selected_area[2]}, height={selected_area[3]}"
        )
