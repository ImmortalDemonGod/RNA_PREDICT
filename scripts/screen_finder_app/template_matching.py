...
from typing import (
    Optional,
    Tuple,
)  # Added Dict for potential future use, Tuple is primary
...
def validate_and_match_template(
    screenshot: np.ndarray, template_path: str, threshold: float
    ...
    if template is None:
        logger.error(
            f"Template image '{template_path}' could not be loaded. Check the path and file integrity."
        )
        return None
    ...
    if (
        ...
        logger.error(
            f"Screenshot dimensions ({screenshot.shape[:2]}) are smaller than template dimensions ({template.shape[:2]}). Matching is not possible."
        )
        return None
    ...
    logger.debug(
        f"Template matching result: Max correlation = {max_val:.4f} at location {max_loc}"
    )
    ...
    if max_val >= threshold:
        ...
        match_location_tuple: Tuple[int, int] = (
            int(match_location[0]),
            int(match_location[1]),
        )
        correlation_score: float = float(max_val)
        logger.info(
            f"Template match found with correlation {correlation_score:.4f} >= threshold {threshold} at {match_location_tuple}"
        )
        # Return the explicitly typed tuple
        return match_location_tuple, correlation_score
    else:
        logger.info(
            f"No template match found above threshold {threshold}. Max correlation was {max_val:.4f}"
        )
        return None
...
if __name__ == "__main__":
    ...
    # Perform matching
    location = validate_and_match_template(
        dummy_screenshot, test_template_path, test_threshold
    )
