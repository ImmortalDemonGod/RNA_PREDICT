...
def hydra_cfg():
    ...
    """
    Creates and returns a Hydra OmegaConf configuration object for testing.
    
    The configuration is initialized from a specified directory, set to use the CPU device, and populated with default diffusion noise schedule and model architecture parameters required for RNALightningModule tests.
    """
...
def test_sample_noise_level_shape_and_device(hydra_cfg):
    ...
    Verifies that `_sample_noise_level` produces a positive tensor of shape `(batch_size,)` on CPU, with all values within the configured minimum and maximum noise schedule bounds.
    """
...
def test_add_noise_shapes_and_broadcasting(hydra_cfg):
    """
    Tests that the _add_noise method correctly adds noise to coordinate tensors, preserving shape and device, and producing non-identical outputs.
    
    Verifies that the noisy coordinates and noise tensors returned by _add_noise have the same shape and device as the input coordinates, and that noise addition alters the input tensor.
    """
    model = RNALightningModule(cfg=hydra_cfg)
    batch_size = 4
    N_atom = 10
...
def test_add_noise_empty_tensor(hydra_cfg):
    """
    Tests that adding noise to empty coordinate tensors returns empty outputs with correct shapes and values.
    """
    model = RNALightningModule(cfg=hydra_cfg)
...
def test_sample_noise_level_extreme_sigma(hydra_cfg):
    """
    Tests that the noise level sampling method respects extreme minimum and maximum sigma values.
    
    Modifies the noise schedule configuration to set very low and high sigma bounds, then verifies
    that all sampled noise levels fall within these specified limits.
    """
