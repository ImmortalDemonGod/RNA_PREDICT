...
def cfg(tmp_path):
    ...
    """
    Creates a minimal Hydra-like configuration and index CSV for RNADataset tests.
    
    Args:
        tmp_path: Temporary directory path provided by pytest for file creation.
    
    Returns:
        An OmegaConf configuration object with data and extraction backend settings, and a CSV file containing a single dataset entry.
    """
    cfg_dict = {
        "device": "cpu",
        "data": {
...
def test_load_angles_uses_extraction_backend(monkeypatch, cfg):
    ...
    """
    Tests that the RNADataset loader uses the configured angle extraction backend when loading angles.
    
    This test monkeypatches the angle extraction and atom parsing functions to intercept the backend argument and avoid file I/O. It asserts that the backend passed to the extraction function matches the value specified in the configuration.
    """
    called = {}
    def fake_extract(*args, **kwargs):
        ...
        Records the 'backend' keyword argument in the shared 'called' dictionary and returns a zero tensor with shape (max_residues, 7).
        """
