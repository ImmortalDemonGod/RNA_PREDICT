...
from rna_predict.pipeline.stageD.diffusion.generator import sample_diffusion
...
@pytest.mark.xfail(reason="Expected to fail due to shape mismatch")
def test_multi_sample_shape_mismatch_memory_efficient():
    """Test multi-sample shape handling with smaller tensors and reduced model config."""
    def get_memory_usage():
    ...
    # Create minimal valid inputs with reduced dimensions
    batch_size = 1
    num_atoms = 5
    num_tokens = 5
    num_samples = 2
    batch_shape = (batch_size, num_samples)  # Define batch_shape for proper broadcasting

    # Create trunk embeddings with smaller dimensions
    s_trunk = torch.randn(batch_size, num_samples, num_tokens, 384)  # Reduced from original size
    s_inputs = torch.randn(batch_size, num_samples, num_tokens, 449)  # Reduced from original size
    z_trunk = torch.randn(batch_size, num_samples, num_tokens, num_tokens, 128)  # Reduced from original size

    # Create input features with smaller dimensions
    input_features = {
        "atom_to_token_idx": torch.zeros((batch_size, num_atoms), dtype=torch.long),
        "ref_pos": torch.randn(batch_size, num_atoms, 3),
        "ref_mask": torch.ones(batch_size, num_atoms),
    }

    # Set up diffusion model with reduced configuration
    diffusion_module = DiffusionModule(
        num_blocks=1,  # Reduced from 4
        num_heads=2,   # Reduced from 16
    )

    # Monitor memory before running diffusion
    pre_diffusion_memory = get_memory_usage()
    print(f"Memory before running diffusion: {pre_diffusion_memory:.2f} MB")
    memory_increase = pre_diffusion_memory - initial_memory
    assert memory_increase < MEMORY_THRESHOLD, f"Memory increase ({memory_increase:.2f} MB) exceeds threshold"

    try:
        # Set timeout to prevent hanging
        with timeout(seconds=30):
            # Run diffusion with reduced number of steps
            noise_schedule = torch.linspace(1.0, 0.0, steps=2)  # Reduced from 20 steps
            x_denoised = sample_diffusion(
            ...
            # Verify output shape
            expected_shape = (batch_size, num_samples, num_atoms, 3)
            assert x_denoised.shape == expected_shape, f"Expected shape {expected_shape}, got {x_denoised.shape}"
        ...
        # Cleanup
        del diffusion_module, s_trunk, s_inputs, z_trunk, input_features
        gc.collect()
...
def test_local_trunk_small_natom_memory_efficient():
    ...
    try:
        ...
        with timeout(seconds=30):
            ...
            # Verify output shape
            assert coords_out.shape == (batch_size, num_atoms, 3), f"Expected shape {(batch_size, num_atoms, 3)}, got {coords_out.shape}"
        ...
        # Cleanup
        del trunk_embeddings, partial_coords, input_features
        gc.collect()
        torch.cuda.empty_cache() if torch.cuda.is_available() else None
        ...
        memory_increase = final_memory - initial_memory
        print(f"Final memory increase: {memory_increase:.2f} MB")
        assert memory_increase < MEMORY_THRESHOLD, f"Final memory increase ({memory_increase:.2f} MB) exceeds threshold"
