...
class MockAtomAttentionEncoder(torch.nn.Module):
    ...
    def forward(self, input_feature_dict, r_l, s, z, **kwargs):
        ...
        a_token = torch.zeros((batch_size, n_sample, n_token, c_token), device=r_l.device)
        q_skip = torch.zeros((batch_size, n_sample, n_atom, 128), device=r_l.device)  # Dummy q_skip
        c_skip = torch.zeros((batch_size, n_sample, n_atom, 128), device=r_l.device)  # Dummy c_skip
        p_skip = torch.zeros((batch_size, n_sample, n_atom, n_atom, 16), device=r_l.device)  # Dummy p_skip
...
def mock_sample_diffusion(*args, **kwargs):
    ...
    input_feature_dict = kwargs.get('input_feature_dict', args[1] if len(args) > 1 else {})
...
basic_patches = [
    ...
    patch('rna_predict.pipeline.stageD.diffusion.diffusion.DiffusionModule', MockDiffusionModule),
]

standard_patches = basic_patches + [
    # Mock the sample_diffusion function
    patch('rna_predict.pipeline.stageD.diffusion.protenix_diffusion_manager.sample_diffusion', mock_sample_diffusion),
    # Mock expand_at_dim
...
comprehensive_patches = standard_patches + [
    ...
    patch('rna_predict.pipeline.stageA.input_embedding.current.transformer.AtomAttentionEncoder', MockAtomAttentionEncoder),
    patch('rna_predict.pipeline.stageA.input_embedding.current.transformer.AtomAttentionDecoder', MockAtomAttentionDecoder),
    patch('rna_predict.pipeline.stageA.input_embedding.current.transformer.DiffusionTransformer', MockDiffusionTransformer),
    patch('rna_predict.pipeline.stageA.input_embedding.current.embedders.FourierEmbedding', MagicMock),
    patch('rna_predict.pipeline.stageA.input_embedding.current.embedders.RelativePositionEncoding', MagicMock),
    patch('rna_predict.pipeline.stageA.input_embedding.current.primitives.LayerNorm', MagicMock),
    patch('rna_predict.pipeline.stageA.input_embedding.current.primitives.LinearNoBias', MagicMock),
    patch('rna_predict.pipeline.stageA.input_embedding.current.primitives.Transition', MagicMock),
]
...
def verify_input_handling(manager, patches):
    ...
    try:
        ...
        override_input_features = {
            ...
            "ref_element": torch.zeros((batch_size, n_atoms, 128), dtype=torch.float32)  # One-hot encoding
        }
...
def verify_output_validation(coords_final):
    ...
    if len(coords_final.shape) == 4:
        print(f"✓ Output has 4 dimensions [B, N_sample, N_atom, 3]")
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Verify ProtenixDiffusionManager component")
    parser.add_argument("--mode", choices=["basic", "standard", "comprehensive"], 
