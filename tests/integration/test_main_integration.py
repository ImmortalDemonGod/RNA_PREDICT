...
def test_main_py_subprocess_execution() -> None:
    ...
    out = result.stdout
    assert "Running demo_run_input_embedding()..." in out, (
        "Expected script to print the embedding demo start message."
    )
    assert "Now streaming the bprna-spot dataset..." in out, (
        "Expected script to print the bprna streaming message."
    )
    assert "Showing the full dataset structure for the first row..." in out, (
