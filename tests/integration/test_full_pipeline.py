...
class TestFullPipeline(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            try:
                diffusion_manager = ProtenixDiffusionManager(dummy_diffusion_config, device=self.device)
                self.config.update({
    ...
    def test_adjacency_initialization(self):
        ...
        z_diff = (result_adj["z_embeddings"] - result_no_adj["z_embeddings"]).abs().mean().item()
    ...
    def test_with_stageD_if_available(self):
        ...
        self.assertGreaterEqual(len(result["final_coords"].shape), 2)  # At least [batch?, N*atoms, 3]
