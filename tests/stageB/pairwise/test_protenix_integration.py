...
def test_residue_index_squeeze_fix():
    ...
    assert z_init.shape[0] == N_token and z_init.shape[1] == N_token, f"Expected z_init shape (N_token, N_token, c_z), got {z_init.shape}"

    print("test_residue_index_squeeze_fix passed: no expand() error with (N_token,1) residue_index!")