...
from rna_predict.pipeline.stageA.input_embedding.current.utils import aggregate_atom_to_token
...
def test_ref_space_uid_patch():
    ...
    """
    encoder = AtomAttentionEncoder(c_atom=128, c_atompair=16, c_token=384, has_coords=True)

    B, N_atom = 1, 5
    # Make a minimal input feature dict
    ref_pos = torch.randn((B, N_atom, 3), dtype=torch.float32)
    ref_space_uid = torch.arange(N_atom).unsqueeze(0)  # shape [1, 5], triggers the patch
    input_feature_dict = {
        "ref_pos": ref_pos,
    ...
    try:
        ...
        pytest.fail("AssertionError still triggered after shape patch for ref_space_uid!")
