...
from rna_predict.pipeline.stageA.input_embedding.current.primitives.attention.chunk_utils import (
    KeysValuesChunkParams,  # Import helper if needed for setup/verification
...
def test_process_keys_values_chunks(
    ...
    assert (
        k_trunked.shape == expected_k_shape
    ), f"Expected K shape {expected_k_shape}, got {k_trunked.shape}"
    assert (
        v_trunked.shape == expected_v_shape
    ), f"Expected V shape {expected_v_shape}, got {v_trunked.shape}"

    # 2. Check bias output structure and shapes
    if provide_bias:
        assert attn_bias_list is not None, "Expected bias list, got None"
        assert (
            len(attn_bias_list) == n_k_trunks
        ), f"Expected {n_k_trunks} bias chunks, got {len(attn_bias_list)}"
        ...
        for i, bias_chunk in enumerate(attn_bias_list):
            assert (
                bias_chunk.shape == expected_bias_chunk_shape
    ...
    torch.testing.assert_close(
        k_trunked[..., 0, :first_chunk_len, :], k[..., :first_chunk_len, :]
    )
    torch.testing.assert_close(
        v_trunked[..., 0, :first_chunk_len, :], v[..., :first_chunk_len, :]
    )

    if provide_bias:
        torch.testing.assert_close(
            attn_bias_list[0][..., :first_chunk_len], bias[..., :first_chunk_len]
        )
    ...
    if seq_len_k_override % n_keys_override != 0:
        ...
        assert torch.all(
            k_trunked[..., last_chunk_idx, original_len_last_chunk:, :] == 0.0
        )
        assert torch.all(
            v_trunked[..., last_chunk_idx, original_len_last_chunk:, :] == 0.0
        )

        # Check bias padding (should be -inf)
        if provide_bias:
            assert torch.all(
                attn_bias_list[last_chunk_idx][..., original_len_last_chunk:] == -INF
            )
