...
def test_create_padding_tensor_positive_length(padding_dim: int):
    ...
    padding_length = 3
    padding_tensor = _create_padding_tensor(
        original_tensor, padding_length, padding_dim
    )
...
def test_reshape_query_to_trunk(
    ...
    q_reshaped_original_part = result.trunked_tensor.reshape(
        batch_dims + [expected_total_length, d_head]
    )[..., :n, :]
    ...
    if needs_padding:
        ...
        q_reshaped_padding_part = result.trunked_tensor.reshape(
            batch_dims + [expected_total_length, d_head]
        )[..., n:, :]
...
def test_reshape_bias_for_trunked_query(
    n_q_pad: int, n_queries: int, original_length: int
):
    """
    ...
    assert (
        reshaped_bias.numel()
        == bias_shape[0] * bias_shape[1] * padded_len * bias_shape[3]
    )
...
def test_create_different_dim_bias(
    original_length: int, n_queries: int, n_keys: int, n_k_trunks: int
):
    """
    ...
    for i in range(n_q_trunks):
        ...
        if q_start >= original_length:  # Trunk is fully padding (Line 182)
            ...
            if valid_queries_in_trunk < n_queries:
                assert torch.all(
                    attn_bias_trunked[..., i, valid_queries_in_trunk:, :, :]
                    == -config.inf
                )
...
def test_process_attention_bias_case1_matches_original_q_with_list():
    ...
    attn_bias = torch.randn(
        2, 4, original_q_len, original_k_len
    )  # B, H, Nq_orig, Nk_orig
