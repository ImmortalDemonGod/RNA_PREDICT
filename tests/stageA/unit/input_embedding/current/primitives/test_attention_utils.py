...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            16,
            (1, 16, 16, 64),
        ),  # Split 32 queries into 2 chunks of 16, concat along dim -3 (size 8)
        (
            (2, 4, 64, 128),
            32,
            (2, 8, 32, 128),
        ),  # Split 64 queries into 2 chunks of 32, concat along dim -3 (size 4)
        (
            (1, 1, 10, 20),
            10,
            (1, 1, 10, 20),
        ),  # n_queries_split equals original query dim
        ...
        (
            ...
            12,
            (1, 1, 10, 20),
        ),  # n_queries_split > original query dim (no split)
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            64,
            8,
            AttentionChunkConfig(chunk_size=8, n_chunks_q=4, n_chunks_k=4),
        ),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=128, n_chunks_q=2, n_chunks_k=2),
        ),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=50, n_chunks_q=1, n_chunks_k=1),
        ),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=100, n_chunks_q=1, n_chunks_k=1),
        ),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=64, n_chunks_q=2, n_chunks_k=2),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=40, n_chunks_q=1, n_chunks_k=1),
        ),
        ...
        (
            ...
            64,
            None,
            AttentionChunkConfig(chunk_size=1, n_chunks_q=1, n_chunks_k=1),
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            (2, 4, 1, 16, 32),
            (2, 4, 8, 32),
        ),  # 5D bias (trunked), squeeze dim -3, THEN SLICE to match n_queries_chunk=8 # Corrected expectation
        (
            (2, 4, 8, 32),
            (2, 4, 8, 32),
        ),  # Bias query dim < chunk query dim (needs slicing)
        (
            (2, 4, 1, 8, 32),
            (2, 4, 8, 32),
        ),  # 5D bias query dim < chunk query dim (squeeze + slice)
    ],
)
def test_process_attention_chunk_bias_handling(
    ...
    with patch(
        "rna_predict.pipeline.stageA.input_embedding.current.primitives.attention_utils._attention",
        return_value=torch.randn_like(q_chunk),
    ) as mock_attention:
...
def test_get_attention_bias_priority(
    default_local_attention_inputs, device
):  # Removed mocker
    """
    ...
    with patch(
        "rna_predict.pipeline.stageA.input_embedding.current.primitives.attention_utils.create_local_attn_bias"
    ) as mock_create_bias:
...
def test_get_attention_bias_creation_path(
    default_local_attention_inputs, device
):  # Removed mocker
    """
...
def test_get_bias_slice(bias_ndim, expected_slice_expr, device):
    ...
    result_slice = attention_utils._get_bias_slice(
        local_bias, chunk_idx, start_idx, end_idx
    )
