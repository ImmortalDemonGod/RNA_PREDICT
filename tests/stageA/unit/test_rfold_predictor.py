...
class TestStageARFoldPredictor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.checkpoint_path is None:
            logger.warning("No checkpoint file found. Tests will run without loading weights.")
    ...
    def test_instantiation_with_checkpoint(self):
        ...
        if self.checkpoint_path is None:
            logger.warning("Skipping test_instantiation_with_checkpoint as no checkpoint file was found.")
            return
    ...
    def test_model_weights_loading(self):
        ...
        for name, param in predictor.model.named_parameters():
            self.assertIsNotNone(param.data, f"Model weights not loaded for layer: {name}")
    ...
    def test_device_configuration(self):
        ...
        if torch.cuda.is_available() and self.config.get("use_gpu", True):
            self.assertEqual(str(predictor.device), "cuda", "Device not configured to GPU")
        else:
            self.assertEqual(str(predictor.device), "cpu", "Device not configured to CPU")
    ...
    def test_predict_adjacency_method_exists(self):
        ...
        predictor = StageARFoldPredictor(config=self.config)
        self.assertTrue(hasattr(predictor, 'predict_adjacency'), "predict_adjacency method not found")
        self.assertTrue(callable(predictor.predict_adjacency), "predict_adjacency is not callable")
    ...
    def test_output_validation(self):
        ...
        self.assertTrue(isinstance(output, (np.ndarray, torch.Tensor)), 
        ...
        if isinstance(output, np.ndarray):
            self.assertEqual(output.shape, (N, N), f"Output shape is not [N, N], got {output.shape}")
        else:
            self.assertEqual(output.shape, torch.Size([N, N]), 
        ...
        if isinstance(output, np.ndarray):
            ...
            self.assertTrue(torch.all((output >= 0.0) & (output <= 1.0)), 
                           "Output values are not within the expected range")
            self.assertFalse(torch.any(torch.isnan(output)), "Output contains NaN values")
            self.assertFalse(torch.any(torch.isinf(output)), "Output contains Inf values")
    ...
    def test_basic_operational_functionality(self):
        ...
        try:
            ...
            logger.info(f"StageARFoldPredictor executed successfully. Result shape: {result.shape}")
        except Exception as e:
