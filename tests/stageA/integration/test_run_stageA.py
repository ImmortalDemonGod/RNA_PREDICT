...
from omegaconf import OmegaConf, DictConfig, ListConfig # Import OmegaConf
...
class TestBase(unittest.TestCase):
    ...
    test_dir: str
    @classmethod
    def setUpClass(cls) -> None: cls.test_dir = tempfile.mkdtemp(prefix="run_stageA_tests_")
    @classmethod
    def tearDownClass(cls) -> None: shutil.rmtree(cls.test_dir, ignore_errors=True)
...
class TestDownloadFile(TestBase):
    ...
    def test_existing_non_zip_skips_download(self):
        with open(self.download_path, "wb") as f: f.write(b"Existing data")
        download_file(self.url_regular_file, self.download_path)
        with open(self.download_path, "rb") as f: data = f.read()
    ...
    def test_existing_valid_zip_skips_download(self):
        ...
        with zipfile.ZipFile(zip_path, "w") as zf: zf.writestr("test.txt", "dummy")
    ...
    def test_existing_corrupted_zip_redownloads(self):
        ...
        with open(zip_path, "wb") as f: f.write(b"Not a zip")
    ...
    def test_download_new_file(self, mock_copyfileobj, mock_urlopen):
        mock_response = MagicMock(); mock_response.__enter__.return_value = mock_response
...
class TestUnzipFile(TestBase):
    ...
    def test_valid_zip_extraction(self):
        with zipfile.ZipFile(self.zip_file, "w") as zf: zf.writestr("inside.txt", "Hello!")
        unzip_file(self.zip_file, self.extract_dir)
...
class TestVisualizeWithVarna(TestBase):
    ...
    def test_missing_ct_file(self, mock_popen):
        with open(self.jar_path, "w") as f: f.write("fake jar")
        visualize_with_varna(self.ct_path, self.jar_path, self.out_png)
        mock_popen.assert_not_called()
    @patch("subprocess.Popen")
    def test_missing_jar_file(self, mock_popen):
        with open(self.ct_path, "w") as f: f.write(">Test\n1 A 0 2 0 1\n")
        if os.path.exists(self.jar_path): os.remove(self.jar_path)
        with patch("builtins.print") as mock_print:
            visualize_with_varna(self.ct_path, self.jar_path, self.out_png)
            warning_printed = any("VARNA JAR not found" in call.args[0] for call in mock_print.call_args_list if call.args)
            self.assertTrue(warning_printed)
        mock_popen.assert_not_called()
    @patch("subprocess.Popen")
    def test_normal_visualization_with_resolution(self, mock_popen):
        mock_process = MagicMock(); mock_process.communicate.return_value = (b'output', b'')
        mock_popen.return_value = mock_process
        with open(self.ct_path, "w") as f: f.write(">Test\n1 A 0 2 0 1\n")
        with open(self.jar_path, "wb") as f: f.write(b"\x50\x4b\x03\x04")
...
@pytest.mark.integration
@pytest.mark.parametrize("seq", ["A", "ACG", "ACGUACGUA", "G"*100]) # Added longer seq
def test_predictor_different_sequences(mock_stageA_config, seq: str):
    """Check adjacency sizes scale with sequence length."""
    device = torch.device(mock_stageA_config.device)
    assert os.path.exists(mock_stageA_config.checkpoint_path), \
        f"Test setup failed: Checkpoint missing at {mock_stageA_config.checkpoint_path}"
    predictor = StageARFoldPredictor(stage_cfg=mock_stageA_config, device=device)
...
# Removed TestBuildPredictor class
...
class TestMainFunction(TestBase):
    ...
    def test_main_smoke(self, mock_isfile, mock_visualize, mock_unzip, mock_download, MockPredictor):
        ...
        try:
            ...
            if not os.path.exists("RFold"): os.makedirs("RFold")
            ...
            if os.path.exists("RFold"): shutil.rmtree("RFold")


        # 4. Assertions
        mock_download.assert_called_once_with(ANY, expected_zip_path) # Check download called for zip
        mock_unzip.assert_called_once_with(expected_zip_path, "RFold") # Check unzip called
        ...
        # Visualization call depends on the default value of visualization.enabled in stageA.yaml
        # Assuming it's true by default based on schema
