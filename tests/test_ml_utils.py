...
class TestAtomSelector(unittest.TestCase):
    ...
    def test_basic_selection(self, scn_seq, option, discard_absent):
        ...
        seq_len = len(scn_seq)
        x = torch.randn(seq_len, 14, 3)  # Shape: (seq_len, num_atoms, coords)
        
        result, mask = atom_selector(scn_seq, x, option, discard_absent)
        ...
        # Check that result has the expected shape based on the option
        if isinstance(option, str):
            if option == "backbone":
                self.assertEqual(result.shape[-1], 3)  # 3D coordinates
                self.assertEqual(mask.sum() // seq_len, 3)  # N, CA, C
            elif option == "backbone-with-oxygen":
                self.assertEqual(result.shape[-1], 3)  # 3D coordinates
                self.assertEqual(mask.sum() // seq_len, 4)  # N, CA, C, O
            elif option == "backbone-with-cbeta":
                self.assertEqual(result.shape[-1], 3)  # 3D coordinates
                self.assertEqual(mask.sum() // seq_len, 4)  # N, CA, C, CB
            elif option == "backbone-with-cbeta-and-oxygen":
                self.assertEqual(result.shape[-1], 3)  # 3D coordinates
                self.assertEqual(mask.sum() // seq_len, 5)  # N, CA, C, O, CB
            elif option == "all":
                self.assertEqual(result.shape[-1], 3)  # 3D coordinates
                self.assertEqual(mask.sum() // seq_len, 14)  # All atoms
        else:
            # When option is a tensor mask
            self.assertEqual(result.shape[-1], 3)  # 3D coordinates
            self.assertEqual(mask.shape[-1], 14)  # Mask for all atoms
