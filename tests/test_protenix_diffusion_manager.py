...
class TestProtenixDiffusionManagerInitialization(unittest.TestCase):
    ...
    def test_init_adds_initialization_key_if_missing(self):
        ...
        """
        with patch("rna_predict.pipeline.stageD.diffusion.diffusion.DiffusionModule") as mock_module:
            ...
            self.assertIn("initialization", kwargs, "Expected 'initialization' key to be injected into config")
            self.assertEqual(kwargs["initialization"], {}, "Expected an empty dict as the default 'initialization'")
    ...
    def test_init_respects_given_device(self):
        ...
        with patch("rna_predict.pipeline.stageD.diffusion.diffusion.DiffusionModule"):
            manager = ProtenixDiffusionManager(diffusion_config={"initialization": {}}, device="cuda")
...
class TestProtenixDiffusionManagerTrainDiffusionStep(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("rna_predict.pipeline.stageD.diffusion.diffusion.DiffusionModule", autospec=True)
        self.mock_diffusion_module_cls = patcher.start()
        self.addCleanup(patcher.stop)

        self.mock_diffusion_module_instance = MagicMock()
        self.mock_diffusion_module_cls.return_value = self.mock_diffusion_module_instance

        # Patch the sample_diffusion_training function
        self.sample_diff_train_patcher = patch(
            "rna_predict.pipeline.stageD.diffusion.generator.sample_diffusion_training", autospec=True
        )
    ...
    @given(
        ...
        sampler_params=st.dictionaries(keys=st.text(), values=st.floats(min_value=0.1, max_value=10)),
        N_sample=st.integers(min_value=1, max_value=3),
        diffusion_chunk_size=st.one_of(st.none(), st.integers(min_value=1, max_value=10))
    )
...
class TestProtenixDiffusionManagerMultiStepInference(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_diff_patcher = patch(
            "rna_predict.pipeline.stageD.diffusion.generator.sample_diffusion", autospec=True
        )
...
class TestProtenixDiffusionManagerCustomManualLoop(unittest.TestCase):
    ...
    @given(
        ...
        sigma=st.floats(min_value=0.01, max_value=10.0, allow_infinity=False, allow_nan=False),
    )
...
class TestProtenixDiffusionManagerRoundTrip(unittest.TestCase):
    ...
    def setUp(self):
        patcher_module = patch("rna_predict.pipeline.stageD.diffusion.diffusion.DiffusionModule", autospec=True)
    ...
    def test_round_trip_pipeline(self):
        ...
        self.mock_train_diff.return_value = (x_gt_augment, x_denoised_train, train_sigma)
        ...
        self.assertEqual(len(train_out), 3, "Expected a tuple of length 3 from train_diffusion_step")
