...
def test_angle_format_compatibility_extended():
    ...
    for i in range(len(sequence)):
        ...
        if i % 2 == 0:
            degrees_angles[i] = torch.tensor(
                [300.0, 180.0, 50.0, 85.0, 180.0, 290.0, 0.0]
            )
        else:
            degrees_angles[i] = torch.tensor(
                [-300.0, -180.0, -50.0, -85.0, -180.0, -290.0, 0.0]
    ...
    for i in range(len(sequence)):
        if i % 2 == 0:
            ...
            radians_angles[i] = torch.tensor(
                [-5.0, -3.14, -0.8, -1.5, -3.14, -5.0, 0.0]
            )
    ...
    print("\nComparing results:")
    if (
        result_degrees is not None
        and result_radians is not None
        and result_sincos is not None
    ):
        if not has_nan_degrees and not has_nan_radians and not has_nan_sincos:
            ...
            diff_deg_rad = (
                torch.abs(result_degrees["coords"] - result_radians["coords"])
                .mean()
                .item()
            )
            diff_deg_sincos = (
                torch.abs(result_degrees["coords"] - result_sincos["coords"])
                .mean()
                .item()
            )

            print(
                f"Mean absolute difference between degrees and radians: {diff_deg_rad:.6f}"
            )
            print(
                f"Mean absolute difference between degrees and sin/cos: {diff_deg_sincos:.6f}"
            )
