...
def verify_input_parameters():
    ...
    try:
        run_stageC(sequence=sequence, torsion_angles=torsion_angles)
        print_result("Accepts torsion_angles as PyTorch Tensor", True, 
                    f"Shape: {torsion_angles.shape}")
    except Exception as e:
        print_result("Accepts torsion_angles as PyTorch Tensor", False, f"Error: {str(e)}")
    ...
    if "in DEGREES" in docstring:
        ...
        print_result("Torsion angle format", False, "Could not confirm format from docstring")
...
def verify_dependencies():
    ...
    print_result("final_kb_rna.py exists", final_kb_rna_exists, f"Path: {final_kb_rna_path}")
    ...
    try:
        ...
        print_result("Required functions import correctly", True, 
        ...
        if has_coords_key:
            ...
            if is_tensor:
                ...
                if len(shape) == 3:  # [L, num_atoms_per_res, 3]
                    ...
                    shape_details = f"[L={L}, num_atoms_per_res={num_atoms}, dims={dims}]"
                ...
                print_result("No NaNs or Infs in coordinates", no_nan_inf, 
...
def verify_functionality():
    ...
    try:
        ...
        print_result("Runs with method='mp_nerf'", True, "Simple sequence with zero angles")
    except Exception as e:
...
def verify_torsionbert_integration():
    ...
    try:
        ...
        print_result("TorsionBERT integration with angle_mode='degrees'", True, 
                    f"TorsionBERT output shape: {torsion_angles.shape}")
    except Exception as e:
        print_result("TorsionBERT integration with angle_mode='degrees'", False, f"Error: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            print_result("TorsionBERT integration with angle_mode='sin_cos'", False, 
                        "Expected failure but succeeded. MP-NeRF might be handling sin/cos pairs.")
        except Exception as e:
            print_result("TorsionBERT integration with angle_mode='sin_cos'", True, 
                        f"Expected failure: {str(e)}")
            print("       This confirms that MP-NeRF expects degrees, not sin/cos pairs.")
    except Exception as e:
        print_result("TorsionBERT initialization with angle_mode='sin_cos'", False, f"Error: {str(e)}")

def main():
    """Run all verification tests."""
    print("\nStage C Module (3D Reconstruction) Verification Script")
    print("Function: run_stageC in rna_predict.pipeline.stageC.stage_c_reconstruction\n")
