...
def test_angle_format_compatibility():
    ...
    sincos_angles = torch.zeros(
        (len(sequence), 7)
    )  # 7 angles in sin/cos form (first 7 values)
    for i in range(len(sequence)):
        for j in range(7):
            angle_rad = radians_angles[i, j].item()
            sincos_angles[i, j] = (
                math.sin(angle_rad) if j % 2 == 0 else math.cos(angle_rad)
            )

    result_sincos = run_stageC(
        sequence=sequence, torsion_angles=sincos_angles, method="mp_nerf", device="cpu"
    )
    ...
    if not has_nan_degrees and not has_nan_radians and not has_nan_sincos:
        ...
        diff_deg_rad = (
            torch.abs(result_degrees["coords"] - result_radians["coords"]).mean().item()
        )
        diff_deg_sincos = (
            torch.abs(result_degrees["coords"] - result_sincos["coords"]).mean().item()
        )
