...
bond_length = st.floats(min_value=0.1, max_value=10.0, allow_nan=False, allow_infinity=False)
...
class TestGetAxisMatrix(unittest.TestCase):
    ...
    def test_normalization_false(self):
        ...
        basis_no_norm = massive_pnerf.get_axis_matrix(self.a, self.b, self.c, norm=False)
        # Ensure the normalized and unnormalized bases differ significantly.
        self.assertFalse(torch.allclose(basis_norm, basis_no_norm, atol=1e-5),
    ...
    def test_hypothesis_axis_matrix(self, a, b, c, norm):
        ...
        self.assertEqual(basis.shape, (3, 3), "Hypothesis test: Basis should be of shape (3,3)")
        if norm:
            for idx, vec in enumerate(basis):
                self.assertAlmostEqual(torch.norm(vec).item(), 1.0, places=5,
...
class TestMpNerfTorch(unittest.TestCase):
    ...
    def test_consistency_float_tensor(self):
        ...
        params_float = massive_pnerf.MpNerfParams(self.a, self.b, self.c, l_val, theta_val, chi_val)
        ...
        params_tensor = massive_pnerf.MpNerfParams(self.a, self.b, self.c, l_tensor, theta_tensor, chi_tensor)
        result_tensor = massive_pnerf.mp_nerf_torch(params_tensor)
        
        self.assertTrue(torch.allclose(result_float, result_tensor, atol=1e-5),
                        "Results should be equivalent whether using float or tensor inputs.")

    @given(
        a=point3d, b=point3d, c=point3d,
        bond=st.one_of(bond_length, st.lists(bond_length, min_size=1, max_size=1).map(lambda l: l[0])),
        theta=angle, chi=angle
    )
    ...
    def test_hypothesis_mp_nerf(self, a, b, c, bond, theta, chi):
        ...
        result = massive_pnerf.mp_nerf_torch(params)
        self.assertEqual(result.shape, (3,), "Hypothesis test: Output shape must be (3,)")
        self.assertFalse(torch.isnan(result).any(), "Output must not contain NaN values.")
    ...
    def test_degenerate_vectors(self):
        ...
        result = massive_pnerf.mp_nerf_torch(params)
        self.assertEqual(result.shape, (3,), "Degenerate input should still yield an output shape of (3,)")
    ...
    @patch('rna_predict.pipeline.stageC.mp_nerf.massive_pnerf.torch.clamp', wraps=torch.clamp)
    def test_theta_clamping(self, mock_clamp):
        ...
        self.assertEqual(result.shape, (3,), "Output shape must be (3,) even when clamping is used.")
    ...
    def test_near_zero_cb_vector(self):
        ...
        result = massive_pnerf.mp_nerf_torch(params)
        self.assertEqual(result.shape, (3,), "Output tensor shape should be (3,) even with near-zero cb vector.")
        self.assertFalse(torch.isnan(result).any(), "Output must not contain NaN values.")
...
class TestScnRigidIndexMask(unittest.TestCase):
    ...
    def test_short_sequence(self):
        ...
        self.assertEqual(result.numel(), 0, "Sequences shorter than 3 should return an empty tensor.")
    ...
    def test_hypothesis_rigid_mask(self, seq, c_alpha):
        ...
        self.assertEqual(result.shape[0], 3, "Result tensor should contain exactly 3 indices.")
        self.assertEqual(result.dtype, torch.long, "Indices must be of type torch.long.")
