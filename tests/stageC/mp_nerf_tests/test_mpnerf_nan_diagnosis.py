    ...
    if has_nan or has_inf:
        ...
        if tensor.dim() > 1:
            ...
            if len(nan_indices[0]) > 0:
                logger.error(f"First few NaN positions: {[(nan_indices[i][j].item() for i in range(len(nan_indices))) for j in range(min(5, len(nan_indices[0])))]}")
        return True
    return False
...
def test_hypothesis_1_zero_vector_normalization():
    ...
    c = torch.tensor([2.0, 0.0, 0.0], device="cpu")  # Collinear with b-a, will cause zero cross product
    ...
    logger.info("Calling mp_nerf_torch with collinear vectors that would produce zero cross products")
    ...
    try:
        ...
        if has_nan:
            ...
            logger.info("Zero vector normalization did NOT cause NaNs with these inputs")
            return False
...
def test_hypothesis_2_missing_bond_data():
    ...
    angles_to_test = ["C1'-C2'-C3'", "C2'-C3'-C4'", "C3'-C4'-O4'", "O4'-C1'-C2'", "O3'-P-O5'"]
...
def test_hypothesis_4_zero_angles():
    ...
    if has_nan_zero and not has_nan_aform:
        logger.info("CONFIRMED: Zero angles cause NaNs, but A-form angles work correctly")
        return True
    elif has_nan_zero and has_nan_aform:
        logger.info("Both zero and A-form angles produce NaNs, suggesting another issue")
        return False
    else:
...
def test_hypothesis_5_angle_format():
    ...
    try:
        ...
        logger.info(f"Degrees NaNs: {has_nan_degrees}, Radians NaNs: {has_nan_radians}, Sin/Cos NaNs: {has_nan_sincos}")
