...
def check_tensor(tensor, name, location):
    ...
    if has_nan or has_inf:
        ...
        logger.error(f"{location}: {name} contains {nan_count} NaNs and {inf_count} Infs")
...
def mp_nerf_torch_safe(a, b, c, l, theta, chi):
    ...
    """
    logger.debug(f"mp_nerf_torch_safe inputs: a={a}, b={b}, c={c}, l={l}, theta={theta}, chi={chi}")
    ...
    if not ((-np.pi <= theta) * (theta <= np.pi)).all().item():
        logger.warning(f"theta(s) must be in radians and in [-pi, pi]. theta(s) = {theta}")
