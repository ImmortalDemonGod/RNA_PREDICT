...
class TestRNAPredictorMpNerfNaN(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.assertEqual(self.predictor.stageC_method, "mp_nerf", "Stage C method not set to mp_nerf during init.")
    ...
    def test_predict_submission_with_mpnerf_no_nan(self):
        ...
        print(f"[Test Run] Testing predict_submission for sequence: '{sequence}' with mp_nerf...")
        ...
        try:
            ...
            self.assertIsInstance(submission_df, pd.DataFrame, "Output should be a DataFrame.")
            self.assertEqual(len(submission_df), len(sequence), "DataFrame row count should match sequence length.")

            # Define the coordinate columns to check (only for the first prediction)
            coord_cols = [f'x_{i}' for i in range(1, prediction_repeats + 1)] + \
                         [f'y_{i}' for i in range(1, prediction_repeats + 1)] + \
                         [f'z_{i}' for i in range(1, prediction_repeats + 1)]

            # Check if all required columns exist
            missing_cols = [col for col in coord_cols if col not in submission_df.columns]
            self.assertEqual(len(missing_cols), 0, f"Submission DataFrame is missing columns: {missing_cols}")

            # Extract coordinate data as a NumPy array
            coords_data = submission_df[coord_cols].values.astype(float) # Ensure numeric type
            ...
            if has_nan or has_inf:
                ...
                problematic_rows = submission_df[np.isnan(coords_data).any(axis=1) | np.isinf(coords_data).any(axis=1)]
            ...
            self.assertFalse(has_nan, "NaN values detected in the output coordinates using mp_nerf.")
            self.assertFalse(has_inf, "Infinite values detected in the output coordinates using mp_nerf.")
            ...
            self.fail(f"Predict_submission raised ValueError: {e}")
             ...
             self.fail(f"Predict_submission raised IndexError (check residue_atom_choice): {e}")
        except Exception as e:
    ...
    def test_predict_3d_structure_with_mpnerf_no_nan(self):
        ...
        try:
            ...
            self.assertIsNotNone(coords, "Coordinates tensor not found in result dictionary.")
            self.assertIsInstance(coords, torch.Tensor, "Coordinates should be a PyTorch Tensor.")
            self.assertGreater(coords.numel(), 0, "Coordinates tensor should not be empty.")
            ...
            if has_nan or has_inf:
                 ...
                 problem_mask = torch.isnan(coords).any(dim=-1).any(dim=-1) | torch.isinf(coords).any(dim=-1).any(dim=-1)
                 print(coords[problem_mask])

            self.assertFalse(has_nan, "NaN values detected in the output coordinates tensor from predict_3d_structure with mp_nerf.")
            self.assertFalse(has_inf, "Infinite values detected in the output coordinates tensor from predict_3d_structure with mp_nerf.")

            # Check atom count consistency
            expected_atoms_approx = len(sequence) * 10 # Rough estimate (backbone only)
            self.assertGreater(result_dict.get("atom_count", 0), expected_atoms_approx, "Atom count seems too low.")
